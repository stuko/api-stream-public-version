...
docker run -d --name api-stream  ${DOCKER_OPTIONS} --mount type=bind,source=/tmp,target="$(pwd)"/volume/storm/tmp -e "TZ=Asia/Seoul" ${ENV_HOST} ${ENV_OPT} --restart always -p ${STREAM_PORT}:${STREAM_PORT} -p ${HTTP_PORT}:${HTTP_PORT} -e STREAM_LOG_DEBUG=false -e MIN=512m -e MAX=1024m ${THIS_IP}:${DOCKERHUB_PORT}/api/api-collector-stream-docker
cd ..
