...
(function ($) {
    $.fn.tabullet = function (options) {
        ...
        $(metadata).each(function (i, v) {
            ...
            if (v.readonly !== 'true') {
                $('<td/>').html('<input disabled="true" type="text" name="' + v.map + '" class="' + options.textClass + '"/>').appendTo(insertRow);
            }
            else {
        ...
        $(data).each(function (i, v) {
            ...
            $(metadata).each(function (mi, mv) {
                if (mv.type === 'delete') {
                    var td = $("<td/>")
                        .html('<button disabled="true" class="' + options.deleteClass + '">' + options.deleteContent + '</button>')
