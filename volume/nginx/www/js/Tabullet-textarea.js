...
(function ($) {
    $.fn.tabullet = function (options) {
        ...
        $(metadata).each(function (i, v) {
            ...
            if (v.type === 'edit') {
                var td = $("<td/>")
                    .html('<button disabled="true" class="' + options.saveClass + '">' + options.saveContent + '</button>')
                    .attr('data-tabullet-type', 'save')
                    .appendTo(insertRow);
                td.find('button').click(function (event) {
                    var saveData = [];
                    var rowParent = td.closest('tr');
                    var rowNameChildren = rowParent.find('input');
                    var rowValueChildren = rowParent.find('textarea');
                    $(rowNameChildren).each(function (ri, rv) {
                        saveData[$(rv).attr('name')] =  $(rv).val(); // rowValueChildren.eq(ri).val();
                    });
                    $(rowValueChildren).each(function (ri, rv) {
            ...
            if (v.readonly !== 'true') {
                if(i == 0)$('<td/>').html('<input disabled="true" type="text" name="' + v.map + '" class="' + options.textClass + '"/>').appendTo(insertRow);
                else  $('<td/>').html('<textarea disabled="true" cols="40" rows="3" name="' + v.map + '" class="' + options.textClass + '"></textarea>').appendTo(insertRow);
            }
            else {
                    ...
                    td.find('button').click(function (event) {
                        ...
                        $(rowChildren).each(function (ri, rv) {
                            ...
                            if ($(rv).attr('data-tabullet-readonly') !== 'true') {
                                if(ri == 0 ) $(rv).html('<input type="text" name="' + mapName + '" value="' + v[mapName] + '" class="' + options.textClass + '"/>');
                                else $(rv).html('<textarea cols="40" rows="3" name="' + mapName + '" value="' + v[mapName] + '" class="' + options.textClass + '">'+ v[mapName] +'</textarea>');
                            }
                        });
                    });
                }
